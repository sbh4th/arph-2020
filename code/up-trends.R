...
#  input:    up-race-1999-2018.txt, up-hisp-1999-2018.txt
#  output:   
#  project:  ARPH Life Expectancy
#  author:   sam harper \ 2020-06-03
...
up <- read_tsv(here("data", "upoison-race-aadr-1999-2018.txt"), skip=1, 
...
uph <- read_tsv(here("data", "upoison-hisp-aadr-1999-2018.txt"), skip=1, 
...
lc <- read_tsv(here("data", "liverca-age-1999-2018.txt"), skip=1, 
  col_names=c("notes", "gender", "gcode", "race", "rcode", 
              "age", "acode", "year", "yearcode", "deaths", 
              "pop", "crate"), n_max=1280,
  col_types = "cccccccddddc")

 
lc1 <- lc %>% select(gender, race, acode, year, deaths, pop) %>% 
  ...
  rate = deaths / pop * 100000, cause="Liver cancer")

ggplot(subset(lc1, acode!="15-24yrs" & gender=="Male"), 
       aes(x=year, y=rate, colour=race))  + 
  geom_line() + stheme + facet_wrap(~acode, scales="free") + 
  theme(legend.position = c(1, 0), legend.justification = c(1, 0)) + 
  xlab("") + ylab("") + ggtitle("Age-adjusted liver cancer deaths rates per 100k")
