...
# separate grouping for the total
...
# separate grouping for all causes
tot <- aggregate(rawt$total, list(sex = rawt$sex, race = rawt$race), FUN = sum )
tot$cod <- 15
colnames(tot)[3] <- "total"
...
w <- ggplot(subset(raw15, gender=="Women")) + 
  geom_vline(xintercept = 0, linetype="dotted", colour="grey60") +
  geom_bar(data=subset(raw15, gender=="Women" & total<0), 
           aes(y=codf, weight=total), width=0.5, colour = "#e41a1c", fill="#e41a1c") + 
  geom_bar(data=subset(raw15, gender=="Women" & total>=0), 
           aes(y=codf, weight=total), width=0.5, colour = "#377eb8", fill = "#377eb8") +  
...
# export to file
ggsave(here("figures", "le-cause-decomp-2010-2018.png"), 
