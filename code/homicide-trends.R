...
s <- read_tsv(here("data", "homicide-1999-2018.txt"), skip=1, 
  col_names=c("notes", "gender", "gcode", "race", "rcode", "age", "acode", "year", 
  "ycode", "deaths", "pop", "crate"), n_max=480,  
...
# Hispanics
sh <- read_tsv(here("data", "homicide-hisp-1999-2018.txt"), skip=1, 
  col_names=c("notes", "gender", "gcode", "hisp", "hcode", "age", "acode", "year", 
  "ycode", "deaths", "pop", "crate"), n_max=320,
  col_types = "cccfccfdcddc")
...
hom <- bind_rows(s2, sh2)

# create new race group
hom$raceabb <- recode_factor(hom$racen, 
  `1` = "NHAPI", `2` = "NHB", `3` = "NHW", `4`= "Hispanic")

hom$age <- paste(hom$acode, "yrs", sep=" ")
...
w <- ggplot(subset(hom, gender == "Female"), 
            aes(x=year, y=rate, colour=raceabb)) +
  geom_line(show.legend=F, size=1.25) + facet_wrap(vars(age), ncol=4) + 
  scale_x_continuous(breaks=c(2000, 2010)) +
  xlab("") + ylab("") + 
  ggtitle("Age-specific homicide death rates per 100,000", subtitle="Women") + 
  stheme

m <- ggplot(subset(hom, gender == "Male"), 
            aes(x=year, y=rate, colour=raceabb)) + 
...
ggsave(here("figures", "age-hom-trends.png"), plot=p, height=11, width=9)
